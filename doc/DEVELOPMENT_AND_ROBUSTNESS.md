...
*   **Error Enrichment with Documentation Retrieval**: When the `--enrich-errors` flag is used, the `ImplAgent` can augment its error context with precise, local documentation. If a task fails, the agent can instruct the LLM to use a local `doc-retriever` tool. This tool generates and parses `rustdoc`'s JSON output for the specific crates involved in the error. The structured documentation for the exact types, traits, and methods is then sent back to the LLM in a multi-turn conversation. This gives the agent accurate, version-specific context to solve complex problems like trait bounds errors or incorrect API usage, without relying on external search engines.
